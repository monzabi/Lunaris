...
//        NetworkManager.g.debug("Set listener of {} to {}", new Object[] { this, packetlistener});
	...
	public void dispatchPacket(final Packet<?> packet,
		...
		if (this.channel.eventLoop().inEventLoop()) {
			...
			// note: since the type is not dynamic here, we need to actually copy the old
			// executor code
			// into two branches. On conflict, just re-copy - no changes were made inside
			// the executor code.
			if (flush) {
				choice1 = () -> {
					if (enumprotocol != enumprotocol1) {
	...
	// Paper start
	/*
	 * public void a(SecretKey secretkey) { // Nacho - OBFHELPER
	 * this.setEncryptionKey(secretkey); } public void setEncryptionKey(SecretKey
	 * secretkey) { this.o = true; this.channel.pipeline().addBefore("splitter",
	 * "decrypt", new PacketDecrypter(MinecraftEncryption.a(2, secretkey)));
	 * this.channel.pipeline().addBefore("prepender", "encrypt", new
	 * PacketEncrypter(MinecraftEncryption.a(1, secretkey))); }
	 */
