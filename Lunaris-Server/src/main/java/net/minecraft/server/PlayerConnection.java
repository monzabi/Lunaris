...
import net.monzabi.lunaris.network.Networking;
...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
	...
	/*
	 * A 1.8.8 client is sending KeepAlive packets with 0 id while rendering the
	 * GUIDownloadWorld The server would disconnect the client so must we consider
	 * this issue
	 */
	...
	public void a(PacketPlayInBlockPlace packetplayinblockplace) {
		...
		if (itemstack == null || itemstack.l() == 0) {
			...
			if (WindSpigotConfig.stopDecodingItemStackOnPlace
					...
					: (!ItemStack.matches(this.player.inventory.getItemInHand(), packetplayinblockplace.getItemStack())
							|| always)) {
	...
	public void a(PacketPlayInSpectate packetplayinspectate) {
		...
		if (this.player.isSpectator()) {
			...
			if (entity != null) {
				 ...
				 * (entity.world != this.player.world) { WorldServer worldserver1 =
				 * this.player.u(); WorldServer worldserver2 = (WorldServer) entity.world;
				 * 
				 * this.player.dimension = entity.dimension; this.sendPacket(new
				 * PacketPlayOutRespawn(this.player.dimension, worldserver1.getDifficulty(),
				 * worldserver1.getWorldData().getType(),
				 * this.player.playerInteractManager.getGameMode()));
				 * worldserver1.removeEntity(this.player); this.player.dead = false;
				 * this.player.setPositionRotation(entity.locX, entity.locY, entity.locZ,
				 * entity.yaw, entity.pitch); if (this.player.isAlive()) {
				 ...
				 * this.minecraftServer.getPlayerList().updateClient(this.player); } else {
				 * this.player.enderTeleportTo(entity.locX, entity.locY, entity.locZ); }
				 */
	...
	public void a(IChatBaseComponent ichatbasecomponent) {
		...
		/*
		 * this.minecraftServer.aH(); ChatMessage chatmessage = new
	...
	public void a(PacketPlayInClientCommand packetplayinclientcommand) {
		...
		case PERFORM_RESPAWN:
			if (this.player.viewingCredits) {
				// this.player = this.minecraftServer.getPlayerList().moveToWorld(this.player,
				// 0, true);
...
//        }
		...
		if (this.player.playerInteractManager.isCreative()) {
			...
			boolean flag2 = itemstack == null
					|| itemstack.getItem() != null && (!invalidItems.contains(Item.getId(itemstack.getItem()))
							|| !org.spigotmc.SpigotConfig.filterCreativeItems); // Spigot
		...
		if (worldserver.isLoaded(blockposition)) {
			...
			if (!tileentitysign.b() || tileentitysign.c() != this.player) {
				...
				this.sendPacket(new PacketPlayOutUpdateSign(tileentity.world, packetplayinupdatesign.a(),
						tileentitysign.lines)); // CraftBukkit
				return;
			}
	...
	public void a(PacketPlayInCustomPayload packetplayincustompayload) {
		...
		if (isExploiter) {
			return; // NachoSpigot - ignore if they have exploited
		}
		...
		networking.handlePacket(player.playerConnection, packetplayincustompayload);
